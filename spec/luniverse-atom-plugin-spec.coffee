...
LuniverseAtomPlugin = require '../lib/luniverse-atom-plugin'
...
describe "LuniverseAtomPlugin", ->
  ...
  describe "when the luniverse-signin:present-panel event is triggered", ->
    it "attaches the view", ->
      expect(atom.workspaceView.find('.luniverse-signin-modal')).not.toExist()

      # This is an activation event, triggering it will cause the package to be
      # activated.
      atom.workspaceView.trigger 'luniverse-signin:present-panel'

      waitsForPromise ->
        activationPromise

      runs ->
        expect(atom.workspaceView.find('.luniverse-signin-modal')).toExist()
